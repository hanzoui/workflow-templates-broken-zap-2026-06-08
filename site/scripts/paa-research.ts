...
function extractQuestionsFromText(text: string): string[] {
  ...
  for (const line of lines) {
    ...
    for (const starter of QUESTION_STARTERS) {
      if (trimmed.startsWith(starter + ' ') || trimmed.startsWith(starter.toLowerCase() + ' ')) {
...
async function researchQuery(query: string, verbose: boolean): Promise<SearchResult> {
  if (verbose) {
...
`;
  ...
  const topics = [
    { topic: 'Installation & Setup', keywords: ['install', 'setup', 'requirements', 'download'] },
    { topic: 'Getting Started', keywords: ['beginner', 'first', 'start', 'learn', 'tutorial'] },
    { topic: 'Workflow Management', keywords: ['workflow', 'import', 'export', 'share', 'save'] },
    { topic: 'Model Usage', keywords: ['model', 'checkpoint', 'LoRA', 'SDXL', 'Flux'] },
    { topic: 'Video Generation', keywords: ['video', 'animation', 'animate', 'motion'] },
    { topic: 'Image Enhancement', keywords: ['upscale', 'inpaint', 'img2img', 'fix', 'enhance'] },
    { topic: 'Advanced Features', keywords: ['ControlNet', 'custom node', 'API', 'automation'] },
    { topic: 'Troubleshooting', keywords: ['error', 'fix', 'problem', 'issue', 'not working'] },
  ];
