...
function validateXmlStructure(sitemapPath: string): void {
  ...
  if (!hasXmlDecl) {
    console.warn(`  Warning: ${path.basename(sitemapPath)} missing XML declaration`);
  }
  ...
  if (openUrlset !== closeUrlset) {
    throw new Error(`${path.basename(sitemapPath)} has unbalanced <urlset> tags`);
  }
...
async function validateUrls(urls: string[], concurrency = 10): Promise<ValidationResult[]> {
