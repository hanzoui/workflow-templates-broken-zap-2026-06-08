...
async function main() {
  ...
  if (args.length === 0) {
    console.error('Usage: pnpm tsx scripts/generate-gsc-verification.ts <verification-code>');
    console.error('');
    console.error('Example:');
    console.error('  pnpm tsx scripts/generate-gsc-verification.ts google1234567890abcdef');
    console.error('');
    console.error('The verification code is provided by Google Search Console when you');
  ...
  if (!verificationCode.startsWith('google')) {
    console.warn('Warning: Google verification codes typically start with "google"');
