...
function createSyncedTemplate(template: TemplateInfo, locale: string): SyncedTemplate {
  ...
  if (workflow) {
    estimatedTime = estimateGenerationTime(workflow, template.mediaType, template.name);
...
export function runSync(): void {
  ...
  if (limit) {
    logger.info(`Processing top ${templatesToProcess.length} templates by usage\n`);
  } else {
  ...
  for (const locale of locales) {
    ...
    for (const file of fs.readdirSync(dir)) {
      ...
      if (!allIndexNames.has(name)) {
        ...
        logger.info(`  Removed orphan: ${locale === DEFAULT_LOCALE ? '' : locale + '/'}${file}`);
        orphansRemoved++;
      }
    }
  }
